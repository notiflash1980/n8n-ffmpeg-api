...
def render():
    try:
        ...
        # 🎬 4. GENERAR VIDEO CON EFECTO ZOOM (Ken Burns)
        # Explicación del filtro: 
        # zoompan crea el movimiento. s=720x1280 asegura el formato vertical (TikTok/Reels).
        video_filter = (
            "zoompan=z='min(zoom+0.0015,1.5)':d=125:s=720x1280,"
        ...
        # Usamos subprocess para mayor control en Render
        result = subprocess.run(cmd)

        if result.returncode != 0:
            return jsonify({"status": "error", "message": "fallo en ffmpeg"})

        return jsonify({"status": "ok", "message": "video con zoom generado", "audio": has_audio})
